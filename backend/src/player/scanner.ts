...
const AUDIO_EXTENSIONS = new Set(['.mp3', '.flac', '.ogg', '.m4a', '.opus', '.wav', '.aac', '.wma']);
...
export function scanMusicDir(musicDir: string, database: Database.Database): ScanJob {
  ...
  const allFiles = walkAudioFiles(musicDir);
  ...
  const promise = (async () => {
    try {
      ...
      // Process files in chunks (per-directory transaction)
      for (const filePath of allFiles) {
        ...
        const relativePath = resolve(filePath).slice(musicDirAbs.length + 1);
        ...
        let artistId = artistCache.get(artistName);
        if (!artistId) {
        ...
        // Get or create album
        const albumKey = `${artistId}:${albumTitle}:${year}`;
        let albumId = albumCache.get(albumKey);
        if (!albumId) {
          const existing = database.prepare(
            'SELECT id FROM albums WHERE artist_id = ? AND title = ?'
          ).get(artistId, albumTitle) as { id: number } | undefined;
        ...
        database.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?)
        `).run(albumId, artistId, metadataTitle, trackNumber, relativePath, duration);
        status.foundTracks++;

        // Extract embedded cover art
        if (coverData && coverFormat && albumId) {
          try {
            // Fix C5: sanitize extension (strip non-alphanumeric chars, reject unexpected formats)
            const ext = (coverFormat.split('/').pop() || 'jpg')
              .replace(/[^a-z0-9]/gi, '')
              .toLowerCase() || 'jpg';
            const coverFileName = `${albumId}.${ext}`;
            // Fix L6: use env.coversDir instead of process.env
            ...
            logger.debug({ err: err.message, albumId }, 'Failed to save cover art');
          }
        }
      }
...
function walkAudioFiles(dir: string): string[] {
  ...
  function walk(current: string): void {
    try {
      ...
      logger.debug({ dir: current, err: err.message }, 'Error walking directory');
    }
  }
